...
func (b *Board) PutStone(x int32, y int32, c Color) error {
	...
	for dx := -1; dx <= 1; dx++ {
		for dy := -1; dy <= 1; dy++ {
			...
			b.TurnStonesByDirection(x, y, c, int32(dx),
				int32(dy))
		}
	}
	return nil
...
func (b *Board) CanPutStone(x int32, y int32, c Color) bool {
	...
	for dx := -1; dx <= 1; dx++ {
		for dy := -1; dy <= 1; dy++ {
			...
			// ひっくり返すことのできる石がひとつでもあれば、 石を置ける
			if b.CountTurnableStonesByDirection(x, y, c,
				int32(dx), int32(dy)) > 0 {
				return true
			}
		}
...
func (b *Board) TurnStonesByDirection(x int32, y int32, c Color,
	dx int32, dy int32) {
	nx := x + dx
	ny := y + dy
	for {
